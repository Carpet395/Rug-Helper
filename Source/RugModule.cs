...
public class RugHelperModule : EverestModule {
    ...
    public static RugModuleSettings Settings => (RugModuleSettings)Instance._Settings;
    ...
    protected virtual void RestartSpriteBatch()
    {
        //if (Hires)
        //{
    ...
    private static Effect effectAccent
    {
        get
        {
            ...
            {
                if (!reload && GetShaderHelperEffectsAcc().TryGetValue(effectNameAccent, out var eff)) return eff;
                if (reload) reload = false;
                if (Everest.Content.TryGet($"Effects/{effectNameAccent}.cso", out var effectAsset, true))
                {
                    ...
                    {
                        Effect effect = new Effect(Engine.Graphics.GraphicsDevice, effectAsset.Data);
                        GetShaderHelperEffectsAcc().Add(effectNameAccent, effect);
                        Logger.Log(LogLevel.Info, "RugHelper", "Loading shdaer: " + effectNameAccent.ToString());
                        return effect;
                    }
                    catch (Exception ex)
                    {
    ...
    {
        orig(self); // draw everything normally
                    //var field0 = typeof(OuiChapterPanel).GetField("chapter", BindingFlags.Instance | BindingFlags.NonPublic);
                    //bool chapter = (bool)field0.GetValue(self);
        var field = typeof(OuiChapterPanel).GetField("chapter", BindingFlags.Instance | BindingFlags.NonPublic);
        string chapter = (string)field.GetValue(self);
        //if (Parse)
        //string name = self.Area.ID; //.Name;
        //string arg = SaveData.Instance?.LevelSet ?? "Celeste";
        //string text2 = $"{arg}/{name}";
        //Logger.Log("g", text2);
        // find the specific MTexture you want
        //GFX.Gui[_ModAreaselectTexture("areaselect/title", self)].Draw(self.Position + new Vector2(_FixTitleLength(-60f, self), 0f), Vector2.Zero, self.Data.TitleBaseColor);
        //GFX.Gui[_ModAreaselectTexture("areaselect/accent", self)].Draw(self.Position + new Vector2(_FixTitleLength(-60f, self), 0f), Vector2.Zero, self.Data.TitleAccentColor);
        string text = Dialog.Clean(AreaData.Get(self.Area).Name);
        //if (effect == null)
        //{
            //if (!Everest.Content.TryGet("Effects/Cool/Em.cso", out ModAsset modAsset))
            //{
                //throw new InvalidOperationException("No Em.cso file found!");
            //}
        ...
        //Draw.SpriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.LinearClamp, DepthStencilState.Default, RasterizerState.CullNone, effect, transformMatrix);
        ...
        if (effectName != "" && effectName != null && effectName != string.Empty)
        {
            ...
            if (effect != null)
            {
               ...
               // Logger.Log(LogLevel.Info, "Rug", "Applied standard parameters");
                ...
                Draw.SpriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend, SamplerState.LinearClamp, DepthStencilState.Default, RasterizerState.CullNone, effect, GameplayHudRenderer.GetMatrix());
        ...
        var field2 = typeof(OuiChapterPanel).GetField("spotlightAlpha", BindingFlags.Instance | BindingFlags.NonPublic);
        float light = (float)field2.GetValue(self);
        var field3 = typeof(OuiChapterPanel).GetField("spotlightPosition", BindingFlags.Instance | BindingFlags.NonPublic);
        Vector2 lightpos = (Vector2)field3.GetValue(self);
        var field4 = typeof(OuiChapterPanel).GetField("spotlightRadius", BindingFlags.Instance | BindingFlags.NonPublic);
        float lightrad = (float)field4.GetValue(self);
        if (light > 0f)
        {
